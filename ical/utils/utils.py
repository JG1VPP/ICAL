...
class CERRecorder(Metric):
    ...
    def update(self, indices_hat: List[List[int]], indices: List[List[int]]):
        for pred, truth in zip(indices_hat, indices):
            pred = vocab.indices2words(pred)
            truth = vocab.indices2words(truth)

            self.sum_dist += levenshtein(pred, truth)
            self.sum_norm += len(truth)

    def compute(self) -> float:
        return self.sum_dist / max(self.sum_norm, 1)
...
class FPSRecorder(Metric):
    ...
    def update(self, times_ns: List[int]):
        for time_ns in times_ns:
            self.sum_time += time_ns
            self.sum_norm += 1

    def compute(self) -> float:
        return 1e9 * self.sum_norm / self.sum_time


def smooth_weight_adjustment(
    targets, class_of_interest=3, base_weight=1.0, max_weight=10.0
):
    """
...
def ce_loss(
    ...
    loss = F.cross_entropy(
        flat_hat, flat, weight=weights, ignore_index=ignore_idx, reduction=reduction
    )
    return loss
